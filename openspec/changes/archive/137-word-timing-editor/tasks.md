...
- [x] `app/service.py` (`_run_align_phase`, Change-078-Zuordnungs-Schritt):
...
- [x] Tests: Override-Wort behält manuelle Zeit nach Re-Align; Nicht-Override
      bekommt neue Zeit; Text geändert → Override verworfen (kein Crash)

## 3. Frontend: Tabs „Transkription | Timing"

- [x] `RecordingCard.tsx`: Editor-Bereich bekommt Tab-Leiste (Transkription |
      Timing), aktiver Tab als State; Transkription-Tab = heutiges Verhalten
- [x] i18n-Keys (de/en/pt-BR): `editor_tab_transcription`, `editor_tab_timing`
- [x] Timing-Tab rendert `TimingEditor` (neu), Transkription-Tab `SegmentList`

## 4. Frontend: TimingEditor — Wortliste + Laden

- [x] `src/components/TimingEditor.tsx` (neu): read-only-Wortliste/Transkript
      (Segmente + Wort-Spans); Klick auf ein Wort → aktives Wort + Waveform
      lädt das Wort (Zustand in RecordingCard oder TimingEditor)
- [x] Aktives Wort visuell markiert (Karaoke-Stil, kein Konflikt mit
      `karaoke-active` — eigener CSS-Klasse)

## 5. Frontend: Waveform-Timing-Modus (30 %-Zoom)

- [x] `WaveformPlayer.tsx`: Prop/Variante „Timing-Modus" — `pps =
      0.3 * Breite / Wortdauer`, geclampt (`MAX_TIMING_PPS`, Untergrenze
      fitPps), Seek zum Wortstart, Zoom-Gate Change 100 beachten
- [x] Wortwechsel (Klick) aktualisiert Zoom + Seek ohne Player-Neubau

## 6. Frontend: Marker-Overlay + Drag + PATCH

- [x] Marker-Overlay über `ws.getWrapper()`: Highlight-Bereich + Start-/Ende-
      Handles, px↔Zeit via pps + scrollLeft; Anzeige Start/Ende/Länge
      (Formatierung wie bestehende Zeitcodes)
- [x] Drag-Handles (Pointer-Events, Touch): Clamp an Nachbar-Grenzen +
...
- [x] „Reset"-Button am geladenen Wort: löscht Override-Flag (PATCH mit
      `override: false`), Wort behält Zeit bis zum nächsten Re-Align

## 7. Frontend: SegmentList readOnly-Prop

- [x] `SegmentList.tsx`: `readOnly`-Prop — deaktiviert Doppelklick-Edit,
      Speaker-Dropdown/Rename, Grenz-Drag, +/−/Split, Re-Segmentierung;
      Wort-Klick = `onWordClick`-Callback (Timing-Tab) statt nur Seek
- [x] Im Timing-Tab mit `readOnly` gerendert; Transkription-Tab unverändert

## 8. Frontend-Tests (Vitest)

- [x] Zoom-Formel: `pps = 0.3 * Breite / Wortdauer` inkl. Clamping
      (sehr kurzes / sehr langes Wort)
- [x] px↔Zeit-Mapping (Marker-Position, Handle-Drag bei scrollLeft ≠ 0)
- [x] Marker-Clamp: Handle kann Nachbar-Grenze nicht überschreiten;
      Mindestdauer eingehalten
- [x] PATCH-Payload (start/end/override), Rollback bei Fehler
- [x] `readOnly`: alle Edit-Interaktionen aus, Wort-Klick feuert Callback
- [x] tsc clean + `npm run build` OK

## 9. GUI-Test gegen laufendes System (Playwright)

- [x] Testaufnahme (JS-WAV) uploaden → transkribieren (Align an) → Timing-Tab
      → Wort klicken → Waveform gezoomt (Markierung ≈ Wortbereich) → Handle
      ziehen → PATCH verifizieren (API-Wert + Anzeige) → Re-Align → Override
      behalten
- [x] Mobile-Check (Viewport, Touch-Drag auf Handle)
- [x] Testskripte im Repo-Muster (`/opt/data/perf-prof/`-Stil oder dev-server)

## 10. OpenSpec archivieren (nach Umsetzung)

- [x] Spec-Delta (`changes/137/.../specs/transcription-view/spec.md`) nach
      `openspec/specs/transcription-view/spec.md` anwenden
- [x] `changes/137` nach `changes/archive/` verschieben, Status Archived,
      `specs`-Unterordner entfernen; Projekt-README-Konvention beachten

## 11. Commit, Push, CI

- [x] Commit(s), Push direkt auf main, CI-Watch bis success (Jobs melden)
