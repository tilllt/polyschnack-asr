...
### Requirement: Punctuation + Truecasing (ps-post, punc-dienst)

- **Ablauf:** Nach der ASR-Stufe formatiert der punc-dienst im
  `ps-post`-Container den rohen Transkriptionstext (Satzzeichen +
...
- **Reihenfolge:** ASR (roh) → ps-post (Diar/Align + Punc) →
...
- **Architektur:** `ps-post/` (Supervisor: crispr-diar :5098,
...
- **Akteure:** User transkribiert mit konfiguriertem ps-post.
