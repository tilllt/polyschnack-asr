    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_STORED) as zf:
        for rec in recs:
            ...
            if audio.is_file():
                ...
                # Change 018: Original (falls beim Upload transkodiert)
                # mitliefern — per Glob-Konvention `<stored>.orig.<ext>`.
                for orig in sorted(audio.parent.glob(audio.name + ".orig.*")):
                    zf.write(orig, arcname=f"{folder}/audio/{_sanitize(orig.name)}")
            else:
